...
# put time step into specs file
# put time into energy matrix
...
fig3b, ax_density3b = plt.subplots(figsize=(17*cm,10*cm))
...
ax_gambel.set_title("Relationship between error and smoothing parameter")
ax_gambel.set_xlabel("H")
ax_gambel.set_ylabel("Error")
...
x = x_matrix[d_step,:]
y = y_matrix[d_step,:]
z = density_on_particle_matrix[d_step,:]

ax_density3b.tricontour(x, y, z, levels=14, linewidths=0.5, colors='k')
cntr2 = ax_density3b.tricontourf(x, y, z, levels=14, cmap="RdBu_r")

#fig3b.colorbar(cntr2, ax=ax_density3b)
#ax_density3b.plot(x, y, 'ko', ms=3)
...
ax_density3b.contour(xi, yi, zi, levels=14, linewidths=0.5, colors='k')
cntr1 = ax_density3b.contourf(xi, yi, zi, levels=14, cmap="RdBu_r")
...
ax_energy.hlines(y = 0, xmin=min(energy[:,0]), xmax=max(energy[:,0]), color = 'black', linestyle='--',label = 'axvline - full height')
...
ax_density3b.vlines(x = 0, ymin=0, ymax=5, color = 'black', label = 'axvline - full height')
ax_density3b.vlines(x = 5, ymin=0, ymax=2, color = 'black', label = 'axvline - full height')
ax_density3b.vlines(x = 5, ymin=3, ymax=5, color = 'black', label = 'axvline - full height')
ax_density3b.vlines(x = 10, ymin=0, ymax=5, color = 'black', label = 'axvline - full height')
ax_density3b.hlines(y = 0, xmin=0, xmax=10, color = 'black', label = 'axvline - full height')
ax_density3b.hlines(y = 5, xmin=0, xmax=10, color = 'black', label = 'axvline - full height')

# Scatter plot of errors 
ax_gambel.scatter(gambel_matrix_20[:,0], gambel_matrix_20[:,1])
ax_gambel.scatter(gambel_matrix_50[:,0], gambel_matrix_50[:,1])
#ax_gambel.scatter(logH_Gambel20, logGambel20)
#ax_gambel.scatter(logH_Gambel50, logGambel50)
#ax_gambel.plot(logGambel20,y)
...
fig4.savefig("outputs/Kernel_Estimation_Error.pdf")
...
def histogram(frame):
    ...
    _, _, graph_hist_x, = ax_hist.hist(state_vx,bins=no_hist_bins,density=True,color='#4f41e7',alpha=0.7)
